...
rule trinity_inchworm_chrysalis:
  ...
  params:
    memory="200",memory_slurm="200000"
  threads:
    16
  shell:
    """
...
checkpoint align_reads:
  ...
  conda:
    "envs/trinity_utils.yaml"
  params:
    memory="100",memory_slurm="100000"
  threads:
    16
  shell:
    """
...
rule rfam_parallel:
  ...
  conda:
    "envs/rfam.yaml"
  params:
    memory="8",memory_slurm="8000" # increased memory from 2 to 8 becuase it was not sufficient
  threads:
    2
  shell:
    """
...
rule orthodb_blastp_parallel:
  ...
  shell:
    """
    blastp -query {input[fasta]} -db OrthoDB/BlastDB/db -num_threads {threads} -evalue {config[e_value_threshold]} -task blastp-fast -max_hsps 1 -max_target_seqs 1 -outfmt '6 qseqid stitle bitscore' -out {output.output} &> {log}
    """
...
rule kallisto:
  ...
  conda:
    "envs/trinity_utils.yaml"
  params:
    memory="16",memory_slurm="16000" # increased memory from 2 to 8 since it was not sufficient
  threads:
    8
  shell:
    """
...
rule download_orthodb:
   """
   Downloads the 2 useful files (for the moment) of OrthoDB and unzip
   """
   output:
   ...
   shell:
     """
     wget --directory-prefix OrthoDB "https://data.orthodb.org/download/odb11v0_all_fasta.tab.gz" &> {log}
     wget --directory-prefix OrthoDB "https://data.orthodb.org/download/odb11v0_genes.tab.gz" &> {log}
  ...
  run:
    ...
    while True:
      ...
      if l1=="" or l2=="" or l3=="":
        ...
        break
      t=l1.split("\t")
      if t[-1]!='hypothetical protein\n' and (not t[-1].isupper()) and ("uncharacterized" not in t[-1]):
        db.write(l2[:-1]+"\t"+t[-1])
        db.write(l3)
...
rule annotated_fasta:
  ...
  run:
    ...
    with open(log[0], "w") as log_handle:
      ...
      print ("Loading orthodb results from", input["orthodb_results"], file=log_handle)
